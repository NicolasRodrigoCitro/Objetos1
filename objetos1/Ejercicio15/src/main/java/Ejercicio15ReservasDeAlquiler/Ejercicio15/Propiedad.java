...
public class Propiedad {
	...
	public boolean estoyDisponible(DateLapse periodo) {
		return this.reservas.stream().allMatch(reserva -> reserva.noEstaDentroDelPeriodo(periodo)) || this.reservas.isEmpty();
	}
	...
	public void eliminarReserva(Reserva unReserva) {
		if (this.reservas.contains(unReserva)) {
			if (unReserva.getEstaReservadoAntesDe(LocalDate.now())) {
				this.reservas.remove(unReserva);
			}
		}
